    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        public void HandleCollision(GameObject obj, Collision c)
        {
			...
			if (playerHealth != null) {
				...
				if (player.GetComponent<PlayerID> ().playerUniqueName != ownerName) {
		...
		[Command]
		void CmdTellServerWhoGotShot (string uniqueName, int damage) {
			GameObject playerGotShot = GameObject.Find (uniqueName);
			playerGotShot.GetComponent<PlayerHealth> ().TakeDamage (damage);
		}
    }
